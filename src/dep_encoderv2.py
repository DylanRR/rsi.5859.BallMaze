...
class Encoder:
  def __init__(self, leftPin, rightPin):
    ...
    self.__state_history = ["00", "00", "00", "00", "00"]
  ...
  def ISR2(self):
    ...
    self.__lastChangeTime = time.time()                                         # Update the last change time
    self.__calcSpeed()
  ...
  def ISR3(self):
    ...
    self.__state_history.append(newState)
     # Check for consistent patterns in the state history
    if self.__state_history == ["00", "01", "11", "10", "00"]:
        self.direction = True  # Clockwise
    elif self.__state_history == ["00", "10", "11", "01", "00"]:
        self.direction = False  # Counterclockwise
    self.__lastChangeTime = time.time()                                         # Update the last change time
    self.__calcSpeed()
  ...
  def ISR4(self):
    ...
    p2 = self.rightPin.value                                              # Get the current right pin value
    newState = "{}{}".format(int(p1), int(p2))                                   # Create a new state based on the current pin values
    p1a = self.leftPin.value                                               # Get the current left pin value
    p2a = self.rightPin.value                                              # Get the current right pin value
    newestState = "{}{}".format(int(p1a), int(p2a))   
    ...
    action = transitions.get(newState, {}).get(newestState, lambda: None)      # Get the action to perform based on the current and new state, default to doing nothing if transition not defined
